...
from prompt import query_generate_prompt , query_regenerate_prompt , query_result_explanation_prompt
...
class DatabaseManager:
    ...
    def get_connection_form(self, db_type: str) -> Dict[str, Any]:
        ...
        if db_type == "SQLite":
            return {
                'db_name': st.sidebar.text_input("Database Name", value="company.db"),
                'db_host': None,
                'db_port': None,
                'db_user': None,
                'db_password': None
...
class QueryGenerator:
    ...
    def generate_query(self, question: str, schema: str) -> str:
        ...
        # return "DELETE FROM teacher seect"
        # return "SELECT   name FROM   Student;"

        prompt= query_generate_prompt.format(schema=schema)
        try:
            response = self.model.generate_content(prompt + "\n\nUser Question: " + question)
            return response.text
        except Exception as e:
            return "error in query generation:"+str(e)
        
    
    def regenerate_query(self,question,schema,old_query):
        prompt=query_regenerate_prompt.format(schema=schema,previous_query=old_query,question=question)
        # print("Prompt: ",prompt)
        try:
            response = self.model.generate_content(prompt)
            return response.text
    ...
    def result_explanation(self , query, query_results):
        prompt = query_result_explanation_prompt.format(query_results=query_results, sql_query=query)
        response  = self.model.generate_content(prompt)
        # print("Eplanation",response.text)
        return response.text
    ...
    def _get_prompt_template(self, schema: str) -> str:
        ...
        return query_generate_prompt.format(schema=schema)
